...
export default function AuctionDetailPage() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setAuction((prev) => (prev ? { ...prev, approved: true } : null));
      }
    } catch (error) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        router.push("/admin-panel/manage-auctions");
      }
    } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center space-x-4">
              <Button
                variant="outline"
                onClick={() => router.push("/admin-panel/manage-auctions")}
      ...
      <main className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 space-y-10">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            ...
            <Card className="shadow-lg hover:shadow-2xl transition-shadow duration-300">
              <CardHeader>
                <CardTitle className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Admin Actions</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {!auction.approved && !auction.ended && (
                  <>
                    <Button
                      ...
                      className="w-full text-green-600 hover:text-green-700 border-green-600 hover:bg-green-100 dark:border-green-400 dark:hover:bg-green-900/20"
                    >
                    ...
                    <Button
                      ...
                      className="w-full text-red-600 hover:text-red-700 border-red-600 hover:bg-red-100 dark:border-red-400 dark:hover:bg-red-900/20"
                    >
                ...
                {auction.approved && !auction.ended && (
                  <p className="text-center text-gray-600 dark:text-gray-300">
                    This auction is already approved.
                  </p>
                )}
                {auction.ended && (
                  <p className="text-center text-gray-600 dark:text-gray-300">
                    This auction has ended. No actions available.
                  </p>
                )}
              </CardContent>
            </Card>
